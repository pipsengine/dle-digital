...
export default function HrisError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => {
    console.error('[HRIS route error]', error);
    ...
    <div className="min-h-[60vh] flex items-center justify-center px-6">
      <div className="max-w-lg w-full rounded-2xl border border-slate-200 bg-white p-8 shadow-sm text-center">
        <h1 className="text-xl font-extrabold text-slate-900">
          {chunkError ? 'Stale app bundle detected' : 'HRIS page failed to load'}
        </h1>
        {chunkError ? (
          <ol className="mt-4 space-y-2 text-left text-sm font-semibold text-slate-600 list-decimal list-inside">
            ...
            <li>Click <strong>Reload page</strong> below.</li>
          </ol>
        ) : (
        ...
        <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
          {chunkError ? (
            ...
            <button type="button" onClick={() => reset()} className="px-4 py-2 rounded-xl bg-slate-900 text-white text-sm font-extrabold hover:bg-slate-800">
              Try again
            </button>
          )}
          <Link href="/hris" className="px-4 py-2 rounded-xl border border-slate-200 text-sm font-extrabold text-slate-700 hover:bg-slate-50">
            Back to HRIS
          </Link>
        </div>
      </div>
