...
type TaxBand = { id: string; sequence: number; label: string; bandAmount: number | null; rate: number };
type TaxComponent = { id: string; label: string; enabled: boolean; preTax: boolean; calculationBasis: string; rate: number; monthlyCap?: number | null; annualCap?: number | null; minimumMonthlyBase?: number | null; appliesTo?: string[]; requiresEmployeeEvidence?: boolean; priority: number };
type TaxVersion = {
  id: string;
  name: string;
  ...
  notes: string;
  taxBands: TaxBand[];
  statutoryDeductions: TaxComponent[];
  reliefs: TaxComponent[];
  stateRules: Array<Record<string, any>>;
...
export default function TaxPayeClient({ initialNow }: { initialNow: string }) {
  ...
  const [tab, setTab] = useState<'employees' | 'bands' | 'deductions' | 'reliefs' | 'changes'>('employees');
  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-black tracking-tight text-slate-950">Tax PAYE</h1>
              <p className="mt-1 max-w-4xl text-sm font-semibold text-slate-600">
                Configurable Nigeria PAYE engine for Tax Act 2025 rules, statutory deductions, reliefs, effective dates, audit history, and future tax changes.
              </p>
            </div>
          </div>
          <div className="mt-3 flex flex-wrap items-center gap-2">
            ...
            <span className="rounded-full bg-violet-100 px-3 py-1 text-xs font-extrabold text-violet-800">Version: {version?.name || 'Loading'}</span>
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <MetricCard label="Annual Chargeable Income" value={money(payload?.summary.annualChargeableIncome, canViewMoney)} detail={`${number(payload?.summary.employees || 0)} employees assessed`} icon={Scale} tone="violet" />
        <MetricCard label="Annual PAYE" value={money(payload?.summary.annualPaye, canViewMoney)} detail="Progressive annual PAYE liability" icon={Banknote} tone="red" />
        <MetricCard label="Monthly PAYE" value={money(payload?.summary.monthlyPaye, canViewMoney)} detail="Monthly payroll tax deduction" icon={CalendarClock} tone="blue" />
        <MetricCard label="Pre-tax Deductions" value={money(payload?.summary.annualPreTaxDeductions, canViewMoney)} detail={`${money(payload?.summary.annualReliefs, canViewMoney)} configured reliefs`} icon={ShieldCheck} tone="green" />
      </div>
      <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-3">
        <MetricCard label="Ready" value={number(payload?.summary.ready || 0)} detail="PAYE records with no exceptions" icon={BadgeCheck} tone="green" />
        <MetricCard label="Review" value={number(payload?.summary.review || 0)} detail="Configuration or payroll data review" icon={FileText} tone="amber" />
        <MetricCard label="Blocked" value={number(payload?.summary.blocked || 0)} detail={`${number(payload?.summary.exceptionCount || 0)} total exception flags`} icon={AlertTriangle} tone={(payload?.summary.blocked || 0) ? 'red' : 'green'} />
      </div>
          ...
          <div className="border-b border-slate-100 p-4 sm:p-5">
            <div className="grid grid-cols-1 gap-2 lg:grid-cols-[1fr_180px_150px]">
              <div className="relative">
                ...
                <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search employee, department, payroll group, state" className="h-10 w-full rounded-xl border border-slate-200 bg-white pl-9 pr-10 text-sm font-semibold outline-none focus:border-dle-blue focus:ring-2 focus:ring-dle-blue/20" />
          ...
          <div className="overflow-x-auto">
            <table className="min-w-[1080px] w-full divide-y divide-slate-100">
              <thead className="bg-slate-50">
                <tr>{['Employee', 'Group', 'State', 'Gross', 'Pre-tax Ded.', 'Reliefs', 'Chargeable Income', 'Annual PAYE', 'Monthly PAYE', 'Status'].map((header) => <th key={header} className="px-4 py-3 text-left text-xs font-black uppercase tracking-normal text-slate-500">{header}</th>)}</tr>
              </thead>
              <tbody className="divide-y divide-slate-100 bg-white">
                {filtered.map((record) => (
                  <tr key={record.employeeId} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 text-sm font-bold text-slate-700">{record.taxState}</td>
                    <td className="px-4 py-3 text-sm font-black text-slate-900">{money(record.annualGrossIncome, canViewMoney)}</td>
                    <td className="px-4 py-3 text-sm font-bold text-slate-700">{money(record.annualPreTaxDeductions, canViewMoney)}</td>
                    <td className="px-4 py-3 text-sm font-bold text-slate-700">{money(record.annualReliefs, canViewMoney)}</td>
                    <td className="px-4 py-3 text-sm font-black text-slate-900">{money(record.annualChargeableIncome, canViewMoney)}</td>
                    <td className="px-4 py-3 text-sm font-black text-slate-900">{money(record.annualPaye, canViewMoney)}</td>
                    <td className="px-4 py-3 text-sm font-black text-slate-900">{money(record.monthlyPaye, canViewMoney)}</td>
                ...
                {!loading && filtered.length === 0 && <tr><td colSpan={10} className="px-4 py-8 text-center text-sm font-bold text-slate-500">No PAYE records match the selected filters.</td></tr>}
              </tbody>
            </table>
          </div>
        </section>
      ...
      {tab !== 'employees' && (
        <section className="mt-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="text-base font-black text-slate-950">{tab === 'bands' ? 'Tax Bands' : tab === 'deductions' ? 'Statutory Deductions' : tab === 'reliefs' ? 'Reliefs' : 'Regulatory Changes'}</h2>
              <p className="mt-1 text-xs font-semibold text-slate-500">Configuration is read from versioned tax tables with effective dates.</p>
            </div>
          ...
          {tab === 'bands' && (
            <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
              {(version?.taxBands || []).sort((a, b) => a.sequence - b.sequence).map((band) => (
                <div key={band.id} className={`rounded-2xl border p-4 ${band.rate === 0 ? toneStyles.green.card : toneStyles.violet.card}`}>
                  <p className="text-xs font-black uppercase tracking-normal text-slate-500">{band.label}</p>
                  <p className="mt-2 text-xl font-black text-slate-950">{band.bandAmount === null ? 'Open-ended' : money(band.bandAmount, true)}</p>
                  <p className="mt-1 text-sm font-bold text-slate-700">{pctFmt.format(band.rate * 100)}% tax rate</p>
                </div>
              ))}
            </div>
          )}
          ...
          {tab === 'deductions' && (
            <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2">
              {(version?.statutoryDeductions || []).map((item) => (
                <div key={item.id} className={`rounded-2xl border p-4 ${item.preTax ? toneStyles.blue.card : toneStyles.slate.card}`}>
                  <div className="flex items-center justify-between gap-3"><p className="font-black text-slate-950">{item.label}</p><span className={`rounded-full px-2.5 py-1 text-xs font-black ${item.enabled ? toneStyles.green.chip : toneStyles.red.chip}`}>{item.enabled ? 'Enabled' : 'Disabled'}</span></div>
                  <p className="mt-2 text-xs font-semibold text-slate-600">{item.calculationBasis}</p>
                  <p className="mt-2 text-sm font-black text-slate-900">Rate {pctFmt.format(item.rate * 100)}% {item.monthlyCap ? `- monthly cap ${money(item.monthlyCap, true)}` : ''}</p>
                </div>
              ))}
            </div>
          )}
          ...
          {tab === 'reliefs' && (
            <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2">
              {(version?.reliefs || []).map((item) => (
                <div key={item.id} className={`rounded-2xl border p-4 ${toneStyles.cyan.card}`}>
                  <p className="font-black text-slate-950">{item.label}</p>
                  <p className="mt-2 text-xs font-semibold text-slate-600">{item.calculationBasis}</p>
                  <p className="mt-2 text-sm font-black text-slate-900">Rate {pctFmt.format(item.rate * 100)}% {item.annualCap ? `- annual cap ${money(item.annualCap, true)}` : ''}</p>
                  <p className="mt-1 text-xs font-semibold text-slate-600">{item.requiresEmployeeEvidence ? 'Employee evidence required' : 'No evidence flag configured'}</p>
                </div>
              ))}
            </div>
          )}
